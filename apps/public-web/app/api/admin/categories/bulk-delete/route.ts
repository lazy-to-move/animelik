...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { adminCategoryBulkDeleteSchema, deleteAdminCategoryByIds } = await import(
      "../../../../../../../api/services/admin-category-service"
    );
    const payload = adminCategoryBulkDeleteSchema.parse(await request.json());
    const result = await deleteAdminCategoryByIds(payload.ids);
