...
export async function GET(request: Request) {
  ...
  try {
    ...
    const { latestSourceQuerySchema, getLatestAnimeFromSource } = await import(
      "../../../../../../../api/services/admin-scraper-read-service"
    );
    const url = new URL(request.url);
    const payload = latestSourceQuerySchema.parse({
      source: url.searchParams.get("source") ?? undefined,
      limit: url.searchParams.get("limit") ?? undefined,
    });
    const result = await getLatestAnimeFromSource(payload);
